...
NORET_TYPE void do_exit(long code)
{
	...
	/* ===  Procenet cleanup ===  */
	{
	   ...
	   struct task_struct *friend_tsk;
	   struct list_head *pos, *n;
	   int i,j;
	   
	   /* mark ourselves deceased in each friend's list */
	   for (i = 0; i < current->friend_count; i++){
	      friend_tsk = find_task_by_pid(current->friends[i].pid);
	      if(!friend_tsk)
		 continue;
	      for (j = 0; j < friend_tsk->friend_count; j++){
		 if (friend_tsk->friends[j].pid == current->pid){
		    friend_tsk->friends[j].alive = 0;
		    break;
		 }
	      }
	   }
	   /* free all pending invitations sent to us */
	   list_for_each_safe(pos, n, &current->invitations){
